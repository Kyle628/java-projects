...
class listmap implements Iterable<Entry<String,intqueue>> {
  ...
  public void insert (String key, int linenr) {
    //Search down the list using curr and prev to locate an insertion point.
    //Quit loop when insertion point is found or at end of list.
    //If the word(key) is not in the list, allocate a new node and insert in correct position.
    //Modify listmap.insert to call queue.insert to increment the word count. 
    //If it's in the list do nothing. 
    node prev = null;
    node curr = head; 
    int cmp = 1;
    while(curr != null) {
